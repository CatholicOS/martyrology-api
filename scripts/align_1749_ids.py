...
for mm_dd, day in ed.items():
    ...
    for el in els:
        if out and not out[-1].rstrip().endswith(('.', '!', '?')):
            ...
            out.append(el)
    day['elogia'] = out
print('fragments merged:', merged_count, '| elogia now:', sum(len(v['elogia']) for v in ed.values()))

# ---- shared text utilities
LATSTOP = {'sancti','sanctae','sancta','sanctus','sanctorum','sanctarum','beati','beatae','beatorum',
 'martyris','martyrum','episcopi','confessoris','confessorum','virginis','virginum','presbyteri',
 'abbatis','diaconi','papae','regis','monachi','item','eodem','ipso','apud','natalis','passio',
 'depositio','commemoratio','die','qui','quae','cuius','eius','cum','sub','post','anno','tempore',
 'temporibus','imperatore','imperatoris','ecclesiae','domini','nostri','christi','deo','deum'}
...
def stem_hit(stem, toks, exact=False):
    for t in toks:
        if t.startswith(stem): return 1.0
        if not exact and len(t) >= 4 and SequenceMatcher(None, stem, t[:len(stem)]).ratio() >= 0.8: return 0.8
    return 0.0
def cap_tokens(raw):
...
RANK6 = {'martyr','episco','confes','virgin','presby','abbati','diacon','papae','regis','ducis',
         'monach','militu','matron','vidua','sacerd','item','eodem','levita','pontif','anacho','eremit'}
def name_zone(raw):
    raw2 = re.sub(r'\b([A-Z][a-z]{1,3}) ([a-z]{2,})\b', r'\1\2', raw)
    out = []
    for m in MARKER.finditer(raw2):
        for t in re.findall(r'[A-Z][a-zA-Z]+', raw2[m.end():m.end()+80]):
            f = fold(t).strip()
            if f and f[:6] not in RANK6: out.append(f)
    return out
...
for mm_dd, day in ed.items():
    for idx, el in enumerate(day['elogia']):
        k = (mm_dd, idx)
        folded[k] = fold(el); ctoks[k] = cap_tokens(el)
        nzone[k] = name_zone(el); sigs[k] = sig_tokens(folded[k])
...
for mm_dd, day in ed.items():
    ...
    for tot, idx, c in pairs:
        if idx in ui or c in uc: continue
        assign[(mm_dd, idx)] = (c, round(tot, 2), 'same-day')
        ui.add(idx); uc.add(c); claimed.add(c)
p1 = len(assign)
...
for mm_dd, day in ed.items():
    for idx in range(len(day['elogia'])):
        k = (mm_dd, idx)
        if k in assign or not nzone[k] or folded[k].lstrip().startswith(('octava','vigilia')): continue
        best = []
        for c in unclaimed:
            ...
            if s_stem < 0.75: continue
            jac = jaccard(sigs[k], la2004sig.get(c, set()))
            if jac < 0.15: continue
            s_text = SequenceMatcher(None, folded[k][:300], la2004f.get(c, '')).ratio()
            if s_text < 0.40: continue
...
HONOR = {'sancti': ('Sanctus', False), 'sancta': ('Sancta', False), 'sanctae': ('Sancta', False),
         'sanctus': ('Sanctus', False), 'sanctorum': ('Sancti', True), 'sanctarum': ('Sanctae', True),
         'beati': ('Beatus', False), 'beatae': ('Beata', False), 'beatorum': ('Beati', True)}
RANKWORDS = {'Martyris','Martyrum','Episcopi','Confessoris','Confessorum','Virginis','Virginum',
 'Presbyteri','Abbatis','Abbatum','Diaconi','Papae','Regis','Reginae','Ducis','Monachi','Monachorum',
 'Militum','Viduae','Sacerdotis','Levitae','Pontificis','Anachoretae','Eremitae','Imperatoris',
 'Comitis','Matris','Item','Eodem','Ipso','Apud','Sancti','Sanctae','Sanctorum','Beati','Beatae','Ordinis'}
CONNECT = {'et','de','a'}
NUMWORDS = {'duorum':'duo','trium':'tres','quatuor':'quattuor','quattuor':'quattuor','quinque':'quinque',
 'sex':'sex','septem':'septem','octo':'octo','novem':'novem','decem':'decem','viginti':'viginti',
 'triginta':'triginta','quadraginta':'quadraginta','quinquaginta':'quinquaginta','sexaginta':'sexaginta',
 'centum':'centum','plurimorum':'plurimi','multorum':'multi','omnium':'omnes'}
GROUP = {'martyrum':'martyres','militum':'milites','monachorum':'monachi','virginum':'virgines',
 'fratrum':'fratres','confessorum':'confessores','presbyterorum':'presbyteri','sanctorum':'sancti'}
...
def extract_subject(raw):
    ...
    lead = raw2.lstrip()
    # feast leads
    fm = re.match(r'(Circumcisio|Octava|Vigilia|Purificatio|Annuntiatio|Assumptio|Nativitas|Conceptio|Exaltatio|Inventio|Dedicatio|Translatio|Apparitio|Conversio|Commemoratio omnium|Festum|Festivitas)\b', lead)
    if fm:
        words = re.findall(r'[A-Za-z]+', lead)
        keep = []
        for w in words:
            lw = w.lower()
            if lw in ('nostri','eiusdem','ejusdem','quae','qui','cuius'): break
            keep.append(w)
            if len(keep) >= 4: break
        # trim trailing connectors
        while keep and keep[-1].lower() in CONNECT: keep.pop()
    ...
    if m:
        ...
        for t in toks:
            ...
            if t[0].isupper():
                ...
                break
            if len([n for n in names if n[0].isupper()]) >= 3: break
        while names and names[-1] in CONNECT: names.pop()
        ...
        slug = '-'.join(parts)
        subject = ('Sancti ' if not marker.startswith('beat') else 'Beati ') + ' '.join(p.capitalize() for p in parts[:2])
        return slug, subject
    ...
    for idx, el in enumerate(day['elogia']):
        ...
        if k in assign:
            ...
            dep_entries.append({'id': cid, 'month': m, 'day': d, 'entry': idx+1,
                               'deprecated': True, 'attested_in': 'martyrologium_romanum_1749',
                               'subject_la': subject})
            sidecar[cid] = {'method': 'coined-deprecated', 'score': None}
        obj[cid] = el
    aligned[mm_dd] = {'titulus': day['titulus'], 'elogia': obj,
...
with open(ED_DIR / 'alignment.json', 'w', encoding='utf-8') as f:
    json.dump({'$comment': 'Per-ID alignment provenance: same-day / cross-day (matched to a '
               'current CRMEDR ID) or coined-deprecated (coined with deprecated:true in the '
               'CRMEDR). Scores are matcher totals; all draft.', 'ids': sidecar},
