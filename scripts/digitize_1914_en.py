...
HDR = re.compile(
    r"^.{0,8}\b(?:Che|The|Cbe|Ehe|Ghe|che|he|[CGE]he)\s+(\S{2,25})"
    r"\s+(?:Dap|Day|Davy|Dav|Bay|Pap|day|Dan|Dayp|Duy|Oap)\s+of\s+(\S{3,12})",
    re.M,
)
RUNHEAD = re.compile(
    r'^\s*[|:.;\-\'"“”]*\s*\d*\s*(?:JANUARY|FEBRUARY|MARCH|APRIL|MAY|JUNE|JULY'
...
def main():
    ...
    for i, m in enumerate(matches):
        ...
        parts = [
            p.strip(" .")
            for p in re.split(r'[.,;]?\s*[—–]+\s*(?=[A-Z(“"])', body)
            if p.strip(" .—-")
        ]
        parts = [p + ("." if not p.endswith(".") else "") for p in parts]
        days[(mon, d)] = {
            "titulus": f"The {ORDINALS[d - 1]} Day of {MONTH_EN[mon - 1]}",
            "elogia": parts,
        }
    missing = [
        (mo + 1, dd)
        for mo in range(12)
        for dd in range(1, DAYS[mo] + 1)
