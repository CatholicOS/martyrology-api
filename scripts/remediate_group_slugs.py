...
def deaccent(s):
    ...
    s = unicodedata.normalize('NFKD', s)
    return ''.join(c for c in s if not unicodedata.combining(c)).replace('æ','ae').replace('œ','oe')
def foldslug(s):
    s = unicodedata.normalize('NFKD', s)
    s = ''.join(c for c in s if not unicodedata.combining(c))
    return re.sub(r'[^a-z]+', '-', s.lower().replace('æ','e').replace('œ','e').replace('j','i')).strip('-')
...
texts = {}
for m in range(1,13):
    texts.update(json.load(open(TEXTS/'data'/'editions'/'martyrologium_romanum_2004'/f'{m:02d}.json', encoding='utf-8')))
def _mine():
    ...
    for e in current:
        t=texts.get(e['id'])
        if not t: continue
        slugtoks=[p for p in e['id'].split('-',1)[1].split('-') if len(p)>=4 and p not in SLUGSTOP and not p.isdigit()]
        ttoks=fold(t).split()
        for st in slugtoks:
            ...
            for tt in ttoks:
                ...
                if cp>best_cp: best_cp,best_tt=cp,tt
            if best_tt and best_cp>=min(5,len(st)-1) and best_tt!=st:
                pairs[best_tt][st]+=1
    out={}
    for form,cnt in pairs.items():
        best,n=cnt.most_common(1)[0]
        if len(cnt)==1 or n>=2*cnt.most_common(2)[1][1]: out[form]=best
    return out
lemma=_mine()
LEMMA_MANUAL = {'fidei':'fides','spei':'spes','caritatis':'caritas','manuelis':'manuel',
 'sosthenis':'sosthenes','zenaidis':'zenais','irenes':'irene','aristsei':'aristaeus',
 'socratis':'socrates','epistemis':'episteme'}
def lemmatize(tok):
    if tok in LEMMA_MANUAL: return LEMMA_MANUAL[tok]
    if tok in lemma: return lemma[tok]
    if tok.endswith('ae'): return tok[:-2]+'a'
    if tok.endswith('ii'): return tok[:-2]+'ius'
    if tok.endswith('onis'): return tok[:-4]+'o'
    if tok.endswith('oris'): return tok[:-4]+'or'
    if tok.endswith('icis'): return tok[:-4]+'ix'
    if tok.endswith('ntis'): return tok[:-4]+'ns'
    if tok.endswith('etis'): return tok[:-4]+'es'
    if tok.endswith('i') and len(tok)>=4: return tok[:-1]+'us'
    return tok

RANK = {'martyris','martyrum','martyr','episcopi','episcopus','confessoris','confessorum',
 'virginis','virginum','presbyteri','presbyterorum','abbatis','abbatum','diaconi','diaconorum',
 'papae','regis','reginae','ducis','monachi','monachorum','militis','militum','viduae','matronae',
 'sacerdotis','levitae','pontificis','anachoretae','eremitae','imperatoris','comitis','matris',
 'martyres','virgines','confessores','presbyteri','monachi','eunuchi','eunuchus','eunuchorum',
 'fratrum','fratris','fratram','sororum','sororum','puerorum','pueri','puellae','puellarum','clerici',
 'conjugum','sociorum','filiorum','mulierum','germanorum','virorum','adulescentium','infantium',
 'item','eodem','ipso','ordinis','religiosae','centurionis','ac','atque',
 'senis','senioris','iunioris','junioris','maioris','minoris','magni','cognomento','dicti','eius','eiusdem'}
CLASS = {'martyrum':'martyres','militum':'milites','monachorum':'monachi','virginum':'virgines',
 'fratrum':'fratres','confessorum':'confessores','presbyterorum':'presbyteri','sororum':'sorores',
 'puerorum':'pueri','sanctorum':'sancti','sanctarum':'sanctae'}
MARK = re.compile(r'\b(sanctorum|sanctarum|beatorum|beatarum|sancti|sanctae|sanctae|beati|beatae)\b', re.I)
CONNECT = {'et','ac','atque'}
STOPCAP = {'hic','haec','qui','quae','quod','cum','nam','sed','ibi','ibidem','nonnulli','plures',
 'alii','aliorum','postea','deinde','tunc','ubi','sub','inter','apud','natalis','passio','item',
 'eodem','ipso','ipsa','beatus','beata','sanctus','sancta','ceterorum','ceteri','omnium','multorum'}

FUNCWORDS = {'in','ad','ac','et','ex','ob','ut','de','non','sub','cum','per','hic','qui','quae',
 'item','apud','ibi','nam','sed','iam','seu','vel','ab','die','quo','quos','quae','haec','a'}
...
def repair_ocr(s):
    ...
    s = re.sub(r'\bsanct([oa])\s*r?\s*u\s*m\b', lambda m: 'sanct'+m.group(1)+'rum', s, flags=re.I)
    s = re.sub(r'\bbeat([oa])\s*r?\s*u\s*m\b', lambda m: 'beat'+m.group(1)+'rum', s, flags=re.I)
    for w in ('Martyrum','Martyris','Virginum','Monachorum','Presbyterorum','Diaconorum','mulierum',
              'fratrum','germanorum','sororum','Militum'):
...
def desplit(s):
    ...
    return re.sub(r'\b([A-Z][a-z]{1,3}) ([a-z]{2,})\b',
                  lambda mo: mo.group(0) if (mo.group(1).lower() in FUNCWORDS or
                                             mo.group(2).lower() in JOIN2_STOP)
                             else mo.group(1)+mo.group(2), s)
...
def persons(raw):
    ...
    for t in toks:
        ...
        if t[0].isupper():
            ...
            if expect:
                out.append(lemmatize(tf)); expect = False
...
def honor(marker, n, first_lemma):
    ...
    if n >= 2:
        if fem: return ('Beatae' if beat else 'Sanctae')
...
def disp(lem): return lem.replace('-', ' ').title()

def new_id_subject(m, d, ps, marker):
    if len(ps) >= 3:
        slug = f'{ps[0]}-et-socii'; subj = f'{honor(marker,3,ps[0])} {disp(ps[0])} et socii'
    elif len(ps) == 2:
        slug = f'{ps[0]}-et-{ps[1]}'; subj = f'{honor(marker,2,ps[0])} {disp(ps[0])} et {disp(ps[1])}'
    else:
        slug = ps[0]; subj = f'{honor(marker,1,ps[0])} {disp(ps[0])}'
...
for e in current:
    ...
    if lead not in CLASS.values() and len(lead) >= 4 and not lead.isdigit():
        s.add(lemmatize(lead)[:6]); s.add(lead[:6])
    cur_ps[e['id']] = s
...
LEAD = {'martyres','fratres','milites','virgines','monachi','confessores','sorores','presbyteri'}
targets = [e for e in dep_entries if e['id'].split('-',1)[1].split('-')[0] in LEAD]

def name_stems(ps): return set(p[:6] for p in ps)
...
SPECIAL = {
    'mr:1225-martyres-anno': ('MATCH', 'mr:1225-nativitas-domini', ''),   # Christmas Proclamation (Kalenda)
    'mr:0912-martyres-lugduni': ('RESLUG', 'mr:0912-sacerdos', 'Sanctus Sacerdos'),  # St Sacerdos, Bp of Lyon
    'mr:0504-martyres-interritorio': ('RESLUG', 'mr:0504-sacerdos', 'Sanctus Sacerdos'),  # same St Sacerdos
}
...
CROSSDAY_REJECT = {'mr:1116-martyres-africa', 'mr:1221-martyres-tuscia', 'mr:0510-martyres-mediolani'}
decisions = []  # (old_id, kind, new_id, subject, detail)
for e in targets:
    oid = e['id']; m = e['month']; d = e['day']
    if oid in SPECIAL:
        kind, nid, subj = SPECIAL[oid]
        decisions.append((oid, kind + ('-slug' if kind == 'MATCH' else ''), nid, subj, 'special-case')); continue
    txt = id_text.get(oid, (None,None,''))[2]
    ps, marker = persons(txt)
    if not ps:
        decisions.append((oid, 'ANON', oid, e['subject_la'], 'no names extracted')); continue
    ...
    if nid in cur_ids and not occupied(nid, m, d):
        ...
        decisions.append((oid, 'MATCH-slug', nid, '', f'exact slug identity names={ps}')); continue
    ...
    for scope, cand_ids in (('same', by_day.get((m,d), [])), ('cross', list(cur_ids))):
        scored = []
        for cid in cand_ids:
            if occupied(cid, m, d) and cid != oid: continue
    ...
    if best:
        ...
        decisions.append((oid, f'MATCH-{best[0]}', best[4], '',
                          f'shared={best[3]} jac={best[2]:.2f} names={ps}'))
    else:
        decisions.append((oid, 'RESLUG', nid, subj, f'names={ps}'))
# special-case matches: method by day comparison
for oid, (kind, nid, subj) in SPECIAL.items():
...
print('COLLISIONS: reslug-dup=%r  reslug-vs-current=%r  reslug-vs-otherdep=%r' % (dup, clash_cur, clash_dep))
print()
for oid, kind, nid, subj, detail in decisions:
...
for oid, kind, nid, subj, _ in decisions:
    if kind.startswith('MATCH'):
        remap[oid] = nid; drop_dep.add(oid)
    elif kind == 'RESLUG':
        remap[oid] = nid; new_subject[nid] = subj
...
for e in dep_entries:
    ...
    if e['id'] in remap:  # RESLUG
        e = dict(e); e['id'] = remap[e['id']]; e['subject_la'] = new_subject[e['id']]
    new_dep.append(e)
with open(CRMEDR/'data'/'deprecated_ids.json', 'w', encoding='utf-8') as f:
    json.dump(new_dep, f, ensure_ascii=False, indent=1); f.write('\n')
print(f'deprecated_ids.json: {len(dep_entries)} -> {len(new_dep)} ({len(drop_dep)} promoted to current)')
...
for mn, md in months.items():
    ...
    with open(ED/f'{mn:02d}.json', 'w', encoding='utf-8') as f:
        json.dump(md, f, ensure_ascii=False, indent=1); f.write('\n')
with open(ED/'alignment.json', 'w', encoding='utf-8') as f:
    json.dump(align, f, ensure_ascii=False, indent=1); f.write('\n')
