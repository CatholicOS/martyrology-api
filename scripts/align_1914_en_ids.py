...
EN_CAP_STOP = {
    "the", "at", "in", "on", "of", "st", "sts", "ss", "saint", "saints",
    "blessed", "holy", "also", "same", "day", "city", "likewise", "his",
    "her", "our", "lord", "god", "he", "she", "after", "when", "who",
    "this", "there", "item", "further", "moreover", "again", "besides",
    "province", "town", "near", "under", "during", "emperor", "king",
    "governor", "pope", "bishop", "abbot", "church", "order", "monastery",
    "mount", "martyr", "martyrs", "virgin", "confessor", "priest",
    "birthday", "feast", "however", "although", "with", "by", "from", "and",
}

LATSTOP = {
    "sancti", "sanctae", "sancta", "sanctus", "sanctorum", "sanctarum",
    "beati", "beatae", "beatorum", "martyris", "martyrum", "episcopi",
    "confessoris", "confessorum", "virginis", "virginum", "presbyteri",
    "abbatis", "diaconi", "papae", "regis", "monachi", "item", "eodem",
    "ipso", "apud", "natalis", "passio", "depositio", "commemoratio",
    "die", "qui", "quae", "cuius", "eius", "cum", "sub", "post", "anno",
    "tempore", "temporibus", "imperatore", "imperatoris", "ecclesiae",
    "domini", "nostri", "christi", "deo", "deum",
}
ENSTOP = {
    "saint", "saints", "blessed", "martyr", "martyrs", "bishop", "confessor",
    "virgin", "priest", "abbot", "church", "emperor", "birthday", "there",
    "which", "whose", "after", "under", "their", "being", "having", "where",
    "during", "against", "because", "through", "before", "whom", "while",
    "other", "great", "first", "same", "also", "suffered", "death", "faith",
    "christ", "christian", "christians", "persecution", "glorious",
    "crowned", "crown", "received", "obtained", "merited", "command",
    "order", "torments", "tortures", "beheaded", "scourged", "burned",
    "killed", "slain", "thrown", "cast", "prison", "confession", "eminent",
    "renowned", "famous", "illustrious", "sanctity", "miracles", "virtues",
    "heaven", "heavenly", "kingdom", "eternal", "departed", "passed",
    "rested", "peace", "happy", "holily",
}
...
SLUGSTOP = {
    "et", "de", "a", "ab", "in", "cum", "socii", "sociorum",
    "sancti", "sanctae", "sanctorum", "beati", "beatae",
}
...
GROUPNUM = {
    "martyres", "milites", "virgines", "monachi", "fratres", "socii",
    "presbyteri", "episcopi", "confessores", "pueri", "mulieres",
    "innocentes", "martyrum", "duo", "tres", "quattuor", "quinque", "sex",
    "septem", "octo", "novem", "decem", "undecim", "duodecim", "tredecim",
    "viginti", "triginta", "quadraginta", "quinquaginta", "sexaginta",
    "septuaginta", "octoginta", "nonaginta", "centum", "mille", "multi",
    "plurimi", "omnes", "alii", "aliae",
}
...
for m in range(1, 13):
    ...
    texts_en.update(
        json.load(
            open(
                TEXTS / "data" / "editions" / "martyrologium_romanum_2004_en_unofficial" / f"{m:02d}.json",
                encoding="utf-8",
            )
        )
    )
...
def score(k, cid, c):
    ...
    s_first = 0.0
    for (base, _), h in zip(stems, hits):
...
for mm_dd, day in ed.items():
    ...
    for idx in range(len(day["elogia"])):
        ...
        for cid, c in pool.items():
            ...
            if accept(
                folded[k], len(cand_stems[cid]), s_stem, s_first, jac_la, s_en,
                jac_en, "lasig" in c and cid in ids1749,
            ):
...
for mm_dd, day in ed.items():
    m, d = map(int, mm_dd.split("-"))
    left_en = [
        idx for idx in range(len(day["elogia"])) if (mm_dd, idx) not in assign
    ]
    left_cand = [
        cid for cid, _ in la1749.get((m, d), []) if cid not in claimed
    ]
...
print(f"pass1: {p1} | pass2: {p2} | pass3: {p3} | overrides: {ov_used} | total elogia: {len(folded)}")
...
for mm_dd, day in ed.items():
    for idx, el in enumerate(day["elogia"]):
        k = (mm_dd, idx)
        if k not in assign and k not in merges and k not in concl and k not in coins and k not in drops:
...
if dep_entries:
    with open(ROOT / "deprecated_ids_1914.json", "w", encoding="utf-8") as f:
        ...
        f.write("\n")
    print(f"coined deprecated: {len(dep_entries)} -> deprecated_ids_1914.json (merge into crmedr/data/)")
