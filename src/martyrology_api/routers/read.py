...
def resolve_request(
    request: Request, req: ElogiaRequest, locale_q: str | None, edition_q: str | None
) -> Resolution:
    ...
    return resolve(
        ...
        request.app.state.store.available(),
        nation=req.nation,
        year=req.year,
        locale=locale,
    )
...
async def _draft_months(
    request: Request, identity: Identity | None, edition_id: str, month: int
    ...
    if svc is None:
        return None
    if not await request.app.state.authz.check(user_ref(identity), "can_edit", edition_id):
        return None
