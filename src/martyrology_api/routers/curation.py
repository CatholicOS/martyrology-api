...
def valid_topic(topic: str | None = None) -> str | None:
    if topic is not None and not TOPIC_RE.fullmatch(topic):
        raise ApiProblem(
            422,
            "Invalid topic",
            detail=f"'{topic}' is not a valid topic slug (expected ^[a-z0-9][a-z0-9._-]{{0,63}}$).",
...
def _out(receipt: WriteReceipt) -> WriteReceiptOut:
    return WriteReceiptOut(
        branch=receipt.branch, commit_sha=receipt.commit_sha, pr_url=receipt.pr_url
    )
...
async def patch_elogium(
    ...
    svc = _service(request)
    receipt = await run_in_threadpool(
        svc.patch_elogium, identity, edition_id, canonical_id, body.text, topic, if_match
    )
    return _out(receipt)
...
async def patch_edition(
    ...
    receipt = await run_in_threadpool(svc.patch_edition, identity, edition_id, payload, topic)
    return _out(receipt)
...
async def patch_day(
    ...
    payload = {k: getattr(body, k) for k in body.model_fields_set}
    receipt = await run_in_threadpool(
        svc.patch_day, identity, edition_id, int(month), int(day), payload, topic, if_match
    )
    return _out(receipt)
