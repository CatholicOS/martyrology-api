...
class CurationService:
    ...
    def _shape(self, edition_id: str, branch: str) -> str:
        meta = self.backend.read_file(
            self.repo_for(edition_id), branch, self.edition_meta_path(edition_id)
        )
        if meta is not None:
            shape = json.loads(meta[0]).get("shape")
            if shape in ("day-structured", "flat"):
                return shape
        probe = self.backend.read_file(
            self.repo_for(edition_id), branch, self.month_path(edition_id, 1)
        )
        if probe is not None:
            return detect_shape(json.loads(probe[0]))
        return "day-structured"

    def _read_month_raw(self, edition_id: str, month: int, branch: str) -> tuple[dict, str | None]:
        got = self.backend.read_file(
            self.repo_for(edition_id), branch, self.month_path(edition_id, month)
        )
    ...
    def _commit(
        ...
        try:
            ...
            raise ApiProblem(
                409, "Write conflict", detail=str(exc), type_slug="write-conflict"
            ) from exc
    ...
    def create_edition(
        self, identity: Identity, edition_id: str, shape: str, note: str | None, topic: str | None
    ) -> WriteReceipt:
        ...
        if default_meta is not None:
            raise ApiProblem(409, "Edition already exists", type_slug="already-exists")
    ...
    def patch_edition(
        self, identity: Identity, edition_id: str, fields: dict, topic: str | None
    ) -> WriteReceipt:
        ...
        if got is None:
            raise ApiProblem(404, "Edition has no data here", type_slug="unknown-edition-data")
    ...
    def patch_day(
        ...
        if self._shape(edition_id, branch) != "day-structured":
            raise ApiProblem(
                422, "Day edits need a day-structured edition", type_slug="not-day-structured"
            )
        raw, sha = self._read_month_raw(edition_id, month, branch)
        key = str(day)
        if key not in raw:
            raise ApiProblem(404, "No such day in this edition", type_slug="unknown-day")
        ...
        if "order" in payload:
            ...
            if sorted(payload["order"]) != sorted(current):
                raise ApiProblem(
                    422, "Order must be a permutation of the day's ids", type_slug="bad-order"
                )
        ...
        return self._commit(
            ...
            self.month_path(edition_id, month),
            raw,
            f"edit day {month:02d}-{day:02d}",
            topic,
            if_match or sha,
        )

    def _locate(
        self, edition_id: str, cid: str, branch: str
        ...
        raise ApiProblem(404, "Eulogy not present in this edition", type_slug="unknown-eulogy")
    ...
    def put_elogium(
        ...
        if day is None:
            raise ApiProblem(
                422, "A 'day' is required for day-structured editions", type_slug="day-required"
            )
        if not 1 <= day <= DAYS_IN_MONTH[am]:
            raise ApiProblem(
                ...
                "Invalid day",
                type_slug="invalid-payload",
                errors=[f"day {day} is out of range for month {am:02d} (anchor month of '{cid}')"],
            )
        raw, sha = self._read_month_raw(edition_id, am, branch)
    ...
    def read_month_draft(
        self, edition_id: str, month: int, branch: str
    ) -> dict[int, DayData] | None:
