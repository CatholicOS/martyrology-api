...
def client(tmp_path, make_client):
            ...
            "data/editions/martyrologium_romanum_1749/01.json": json.dumps(
                {
                    "1": {
                        "titulus": "t1",
                        "elogia": {"mr:0102-concordius": "Spoleti."},
                        "conclusio": "c",
                    }
                }
            ),
    ...
    c.app.state.authz = Grants(
        {
            ("can_edit", "martyrologium_romanum_1749"),
...
def test_write_requires_auth(client):
    r = client.patch(
        "/api/v1/editions/martyrologium_romanum_1749/elogia/mr:0102-concordius", json={"text": "x"}
    )
    assert r.status_code == 401
    r = client.patch(
        "/api/v1/editions/martyrologium_romanum_2004/elogia/mr:0101-basilius",
        json={"text": "x"},
        headers=AUTH,
    )
...
def test_put_elogium_happy_path(client):
    r = client.put(
        "/api/v1/editions/martyrologium_romanum_1749/elogia/mr:0101-basilius",
...
def test_draft_falls_back_to_published_when_month_file_absent(client):
    ...
    client.patch(
        "/api/v1/editions/martyrologium_romanum_1749/01/01", json={"titulus": "X"}, headers=AUTH
    )
...
def test_put_month_validation_error(client):
    r = client.put(
        "/api/v1/editions/martyrologium_romanum_1749/01",
        json={"1": {"elogia": {"mr:9999-nobody": "x"}}},
        headers=AUTH,
    )
    assert r.status_code == 422
...
def test_create_edition_needs_admin(client):
    r = client.put(
        "/api/v1/editions/martyrologium_romanum_1914_en_unofficial",
        json={"shape": "day-structured"},
        headers=AUTH,
    )
    assert r.status_code == 201
    r2 = client.put(
        "/api/v1/editions/martyrologium_romanum_1749",
        json={"shape": "day-structured"},
        headers=AUTH,
    )
...
def test_patch_edition_explicit_null_removes_key(client):
    ...
    client.put(
        f"/api/v1/editions/{edition_id}",
        json={"shape": "day-structured", "note": "orig"},
        headers=AUTH,
    )
    r = client.patch(f"/api/v1/editions/{edition_id}", json={"note": "temp"}, headers=AUTH)
    assert r.status_code == 200
    r2 = client.patch(f"/api/v1/editions/{edition_id}", json={"note": None}, headers=AUTH)
    assert r2.status_code == 200
    svc = client.app.state.curation
    raw = json.loads(
        svc.backend.read_file(
            "CatholicOS/martyrology-api", "curation/jdoe/edits", svc.edition_meta_path(edition_id)
        )[0]
    )
